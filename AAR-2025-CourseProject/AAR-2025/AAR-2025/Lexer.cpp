            ...
            else if (isalpha(c))
            {
                ...
                else    // Индетификатор (имя переменной, функции и тд)
                {
                    ...
                    if (identId == TI_NULLIDX)  
                    {
                        ...
                        strcpy_s(entry.id, word.c_str());   // Имя индетификатора
                        entry.idtype = IT::V;               // Тип: индетификатор
                        entry.iddatatype = IT::UNDEF;       // Тип данны индетификатора
                        entry.value.vint = TI_INT_DEFAULT;  
                        entry.idxfirstLE = lt.size + 1;     // Какое по счету слово - индефикатор
                    ...
                    LT::Add(lt, lexEntry);  // Добавляем в таблицу лексем
                }
            }
